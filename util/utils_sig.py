...
def augment_hr_and_video(video, signal, scale_factor):
    """
    Args:
        video: Tensor of shape (C, T, H, W)
        signal: Tensor of shape (T,)
        scale_factor: float, e.g., 1.33 to simulate increased HR
    Returns:
        video_scaled: Tensor of shape (C, new_T, H, W)
        signal_scaled: Tensor of shape (new_T,)
    """
    C, T, H, W = video.shape
    new_T = int(T / scale_factor)

    # Resample video (time axis)
    video_scaled = F.interpolate(video.unsqueeze(0), size=(new_T, H, W), mode='trilinear', align_corners=False).squeeze(0)

    # Resample signal (1D)
    signal_scaled = F.interpolate(signal.view(1, 1, -1), size=new_T, mode='linear', align_corners=False).squeeze()

    return video_scaled, signal_scaled
